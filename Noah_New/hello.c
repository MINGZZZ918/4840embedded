 ...
 */
void init_game_state(int num_enemies, int rows) {


    int extra_space, row;
    num_rows = rows;
    enemies_per_row = num_enemies/num_rows;

    // extra_space = SCREEN_WIDTH % (ENEMY_WIDTH*enemies_per_row);

    // for (int i = 0; i < num_enemies; i++) {

    //     game_state.enemies[i].pos_x = game_state.enemies[i].start_x = i*ENEMY_WIDTH+extra_space;
    //     game_state.enemies[i].pos_y = game_state.enemies[i].start_y = 50;
    //     game_state.enemies[i].sprite = 0;
...
int enemy_movement(){
    ...
    enemy *enemy;
    int num_left = 0, row, new_x, new_y;
    float mag;
    bullet *bul;
    spaceship *ship = &game_state.ship;
    srand(time(NULL));
    ...
    if (!moving[0]){
        ...
        for (int i = 0; i < ENEMY_COUNT; i++){
            ...
            if (enemy->active && !enemy->bul.active){
                if (i <= enemies_per_row*row && i > enemies_per_row*(row-1)){
        ...
        enemy = &game_state.enemies[moving[1]];
        ...
        if(enemy->returning){
            ...
            if (enemy->pos_x == enemy->start_x && enemy->pos_y%SCREEN_HEIGHT == enemy->start_y){
    ...
    for (int i = 0; i < ENEMY_COUNT; i++){
        ...
        if (enemy->active){
            ...
            if (!enemy->moving && !enemy->bul.active && abs(game_state.ship.pos_x - enemy->pos_x) < 10){
