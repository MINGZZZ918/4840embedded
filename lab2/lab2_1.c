 ...
 *
 * Name/UNI: Hiroki Endo (he2305), Jingyi Lai (jl6932)
 */
...
int is_my_address(char *buf)
{
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        if (ifa->ifa_addr != NULL && ifa->ifa_addr->sa_family == AF_INET) {
            ...
            if(addr->sin_addr.s_addr==my_addr.sin_addr.s_addr&&addr->sin_port==my_addr.sin_por			result = 1;
...
/*
 * Cursor moves right
 * Will change row,col,buf * Will rolldown to next row
